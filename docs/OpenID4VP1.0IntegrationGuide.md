...
>
> For the older **OpenID4VP Draft-23** / Presentation Exchange flow (`presentation_definition`), see [OpenID4VP Draft-23 Integration Guide](./OpenID4VPIntegrationGuide.md).
...
- **IETF SD-JWT** formats (`vc+sd-jwt`, `dc+sd-jwt`), including selective disclosure where applicable
...
|----------|----------|-------------|
| `client_id` | Yes | Identifier for the verifier. Interpretation depends on Client Identifier Prefix / `client_id_scheme` support in the wallet stack. |
| `client_id_scheme` | Optional | When used by the deployment, supported values align with Draft-23 integrations: `redirect_uri`, `pre-registered`, `did`. OpenID4VP 1.0 also allows prefixes inside `client_id` (for example `redirect_uri:https://…`). |
...
> Do **not** send `presentation_definition` / `presentation_definition_uri` for an OpenID4VP 1.0 DCQL request. Those parameters belong to the [Draft-23 Presentation Exchange](./OpenID4VPIntegrationGuide.md) flow.

## 4. Supported `client_id_scheme` / Client Identifier Values

### 4.1 `redirect_uri`
- No prior registration needed.
- Wallet identifies the verifier based on the redirect / response URI domain.
...
| `availableCredentials` only (no `queryGroups`) | Draft-23 PE mode — see [Draft-23 guide](./OpenID4VPIntegrationGuide.md) |
...
- If a required query cannot be satisfied, the wallet shows a no-match state; the user may still reject the verifier
...
1. Choose your client identification approach (`client_id_scheme` / Client Identifier Prefix).
...
- [OpenID4VP Draft-23 / Presentation Exchange Integration Guide (Inji Web)](./OpenID4VPIntegrationGuide.md)
